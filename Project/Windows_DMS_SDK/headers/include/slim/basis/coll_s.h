...
/* $Header: /Work/suzuki/tank/sw/slim/slim/basis/coll_s.h 7     06/09/08 20:26 Suzuki $ */

#ifndef SLIM_COLL_S_H
#define SLIM_COLL_S_H
...
struct TBagPage_ {
	...
	slim_short fToBeSplit;	/* the place offset to be split into new holder */
	slim_short fFirstFree;	/* the first free holder offset in page */
};
typedef struct TBagPage_ TBagPage;

struct TBagItemHolder_ {
	slim_int fNext;			/* next free holder offset in page */
...
struct TVarBagPage_ {
	slim_short fFree; /* the number of free holders */
	slim_short fFirstFree; /* the in-page-offset of the first free holder */
	/* fFirstFree == -1 means that there is no free holder in the free holder list */
};
...
struct TVarBagFreeHolder_ {
	slim_short fHolderSize; /* If a holder is a used-holder, its holder-size is stored in its pseudo-index */
	slim_short fNextFree;  /* in-page-offset of the next free holder in the free holder list */
};
...
typedef struct TPtrVarBag_ TPtrVarBag;
...
typedef struct TPtrTree_ TPtrTree;

typedef TTreeItemHolder TPtrTreeItemHolder;

struct TPtrTreeVisit_ {
	slim_pix fIndex;	/* this member refers to the item last visited by TXXXTree_VisitYYY() */
};
typedef struct TPtrTreeVisit_ TPtrTreeVisit;

typedef TTreeTraverse TPtrTreeTraverse;

struct TPtrTreeTraverseVisit_ {
	TPtrTree *fTree;
	TPtrTreeTraverse fTrav;
	TPtrTreeVisit fVisit;
};
typedef struct TPtrTreeTraverseVisit_ TPtrTreeTraverseVisit;

typedef void (*TPtrTreeTidyProc)(TPtrTree *self, slim_pix in_pix, slim_opaque in_arg);
